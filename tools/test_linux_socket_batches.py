...
with tempfile.TemporaryDirectory(prefix="leonos-mmsg-") as directory:
    ...
    subprocess.run(["cc", "-std=c11", "-g", "-O1", "-fsanitize=address,undefined",
                    ...
                    "-Ikernel/ntclks/kernel/ntclks/include", "tools/tests/socket_batch_test.c", "-o", output],
                   cwd=root, check=True)
    ...
    subprocess.run(["cc", "-std=c11", "-g", "-O1", "-fsanitize=address,undefined",
                    "-fno-sanitize-recover=all", "-ffunction-sections", "-fdata-sections",
                    "-Wl,--gc-sections", "-fno-pie", "-no-pie", "-Ikernel/ntclks/include", "-Iinclude", "-Ikernel/ntclks/include/uapi",
                    "-Ikernel/ntclks/kernel/ntclks/include", "tools/tests/socket_batch_unix_test.c", "-o", output],
                   cwd=root, check=True)
    subprocess.run([output], cwd=root, check=True, timeout=20)
