...
assert (ROOT / "kernel/ntclks/include/uapi/linux/syscall.h").read_text() == header()
numeric_syscall = re.compile(r"^\s*#\s*define\s+(?:SYS_|__NR_|LINUX_SYS_)\w+\s+(?:0x[\da-fA-F]+|\d+)\b", re.M)
for relative in ("kernel/ntclks/kernel/ntclks/include/ntclks/syscall.h", "userland/runtime/include/leonos/syscall.h"):
    assert not numeric_syscall.search((ROOT / relative).read_text()), relative
headers = sorted((ROOT / "kernel/ntclks/include/uapi").rglob("*.h"))
for language, compiler in (("c", "clang"), ("c++", "clang++")):
    ...
    for source in headers:
        subprocess.run([compiler, "-x", language, "-fsyntax-only", "-Werror",
                        "-I", str(ROOT / "kernel/ntclks/include/uapi"), "-I", str(ROOT / "include"),
