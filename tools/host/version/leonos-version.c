...
int main(int argc, char **argv)
{
    ...
    if (snprintf(version_string, sizeof(version_string), "%s-%s",
            identity.release_version,
            (options.source_id != NULL && options.source_id[0] != '\0') ?
                options.source_id : "unknown") >= (int)sizeof(version_string)) {
    ...
    status = append_line(&out, "#ifndef LEONOS_GENERATED_BUILD_INFO_H\n") != 0 ||
        ...
        append_line(&out, "/* Generated by %s from %s and the supplied source id. Do not edit. */\n\n",
            TOOL_NAME, options.version_file) != 0 ||
