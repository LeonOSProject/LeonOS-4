...
$(PAM_STAMP) $(PAM_LIB) $(PAM_HEADER) $(PAM_LEONOS_MODULE) &: $(AUTH_STAMP) $(LEONOS_AUTH_ARTIFACTS) $(MUSL_STAMP) $(LEONOS_LOCK) $(PAM_INPUTS) $(LEONOS_SRC)/tools/build/pam-stage.sh $(O_META)/pam.sig $(LEONOS_SRC)/userland/pam/pam_leonos_password.c $(LEONOS_SRC)/userland/libc/src/auth_password.c $(LEONOS_SRC)/include/leonos/auth.h
	$(Q)mkdir -p $(dir $(PAM_STAMP)) $(O_LOGS)
	+$(Q)case "$${MAKEFLAGS%% *}" in *n*) exit 0;; esac; sh $(LEONOS_SRC)/tools/build/pam-stage.sh $(LEONOS_SRC) $(abspath $(LEONOS_DEPS_TOOL)) $(LEONOS_LOCK) $(LEONOS_CACHE) \
