...
$$(if $$(LEONOS_PASSIVE),,$$(eval $$(call LEONOS_SIGNATURE_RULE,upstream-$(1))))
upstream_$(1)_products := $$(addprefix $$(UPSTREAM_ROOT)/$(1)/root/,$$(sort $$(upstream_$(1)_outputs)))
$$(UPSTREAM_ROOT)/$(1)/root/.complete $$(upstream_$(1)_products) &: $$(UPSTREAM_TERMINAL_SCRIPT) $$(LEONOS_LOCK) $$(LEONOS_DEPS_TOOL) $$(O_META)/upstream-$(1).sig $$(MUSL_STAMP) $$(LEONOS_MUSL_ARTIFACTS)
	$$(Q)mkdir -p $$(O_LOGS)
	+$$(Q)case "$$$${MAKEFLAGS%% *}" in *n*) exit 0;; esac; sh $$(UPSTREAM_TERMINAL_SCRIPT) $(1) $$(LEONOS_SRC) $$(abspath $$(LEONOS_DEPS_TOOL)) $$(LEONOS_LOCK) $$(abspath $$(UPSTREAM_ROOT))/$(1)/work $$(abspath $$(UPSTREAM_ROOT))/$(1)/root $$(abspath $$(MUSL_SYSROOT)) $$(TARGET_CC) $$(TRIPLE_USER) >$$(O_LOGS)/upstream-$(1).log 2>&1 || { tail -n 50 $$(O_LOGS)/upstream-$(1).log >&2; exit 1; }
	$$(Q)for product in $$(addprefix $$(UPSTREAM_ROOT)/$(1)/root/,$$(upstream_$(1)_primary)); do test -e "$$$$product" || exit 1; touch "$$$$product"; done
	$$(Q)touch $$(UPSTREAM_ROOT)/$(1)/root/.complete
.PHONY: upstream-$(1)
upstream-$(1): $$(UPSTREAM_ROOT)/$(1)/root/.complete $$(upstream_$(1)_products)
endef
$(foreach package,ncurses,$(eval $(call LEONOS_TERMINAL_RULE,$(package))))
leonos-upstream: upstream-ncurses
...
upstream_app_$(1)_inputs := $$(shell find $$(LEONOS_SRC)/third_party/$(if $(filter pleditor,$(1)),pl_editor,$(1)) $$(LEONOS_SRC)/userland/$(if $(filter pleditor,$(1)),apps/pleditor,$(1)) -type f ! -name .git 2>/dev/null | LC_ALL=C sort)
...
$$(addprefix $$(UPSTREAM_APP_DIR)/,$$(sort $$(upstream_app_$(1)_outputs))) &: $$(upstream_app_$(1)_inputs) $$(LEONOS_SRC)/tools/build/upstream-app.sh $$(LEONOS_LOCK) $$(AUTOCONF_H) $$(RUNTIME_SO) $$(RUNTIME_ARCHIVE) $$(MUSL_STAMP) $$(HEADER_EXPORT_MANIFEST) $$(O_META)/upstream-app-$(1).sig
	$$(Q)mkdir -p $$(O_LOGS)
	+$$(Q)case "$$$${MAKEFLAGS%% *}" in *n*) exit 0;; esac; UPSTREAM_DEPS='$$(abspath $$(LEONOS_DEPS_TOOL))' UPSTREAM_UAPI='$$(abspath $$(HEADER_EXPORT_INCLUDE))' UPSTREAM_CFLAGS='$$(LEONOS_OPTIMIZATION_FLAGS)' UPSTREAM_LDFLAGS='$$(LEONOS_LINK_POLICY_FLAGS)' sh $$(LEONOS_SRC)/tools/build/upstream-app.sh $(1) $$(LEONOS_SRC) $$(abspath $$(UPSTREAM_ROOT))/app-$(1) $$(abspath $$(UPSTREAM_APP_DIR)) $$(abspath $$(MUSL_SYSROOT)) $$(abspath $$(RUNTIME_SO)) $$(abspath $$(RUNTIME_ARCHIVE)) $$(abspath $$(O_INCLUDE)) $$(abspath $$(AUTOCONF_H)) $$(TARGET_CC) $$(TARGET_LD) $$(TARGET_AR) >$$(O_LOGS)/upstream-app-$(1).log 2>&1 || { tail -n 40 $$(O_LOGS)/upstream-app-$(1).log >&2; exit 1; }
...
endef
$(foreach package,sl sqlite,$(eval $(call LEONOS_UPSTREAM_APP,$(package))))
leonos-upstream: $(addprefix upstream-app-,sl sqlite)
...
leonos-upstream: upstream-app-cmd
...
endef
$(foreach package,$(UPSTREAM_PACKAGES) ncurses,$(eval $(call LEONOS_UPSTREAM_PRESENCE,$(package))))
